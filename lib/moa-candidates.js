...
export async function runReferencesParallel(references, messages, options = {}, callLlm, onProgress) {
  ...
  references.forEach((slot, i) => {
    ...
    const role = slot?.role_persona || slot?.role || 'general'
    const persona = (role && ROLE_PERSONA_PROMPTS[role])
      ? `\n\n${ROLE_PERSONA_PROMPTS[role]}`
      : ''
      ...
      try {
        ...
        const timeoutPromise = new Promise((_, reject) => {
          const timer = setTimeout(() => {
            bestEffort('candidate timeout abort', () => {
              abortControllers[i].abort(new Error(`Timeout after ${Math.round(timeoutMs / 1000)}s`))
            })
