window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const STYLES = `
      .moa-section { display:flex; flex-direction:column; gap:16px; padding:4px 0; max-width:720px; }
      .moa-section-title { font-size:18px; font-weight:700; color:var(--dsw-alias-label-primary); }
      .moa-section-sub { font-size:13px; color:var(--dsw-alias-label-secondary); margin-top:2px; margin-bottom:8px; }
    ...
    function MoAFormContent({ t, presets, defaultPreset, setDefaultPreset, setPresets, handleSave, saveStatus, isUnavailable }) {
      if (isUnavailable) {
        return React.createElement('div', { className: 'moa-hint' }, t('unavailable'))
      }
    ...
    // Direct section in Settings sidebar (like Subscriptions / Key Rotation)
    function MoASection(props) {
      ...
      return React.createElement(
        ...
        React.createElement('div', { className: 'moa-section-title' }, t('title')),
        React.createElement('div', { className: 'moa-section-sub' }, t('description')),
        React.createElement(MoAFormContent, {
          t,
          presets,
          defaultPreset,
          setDefaultPreset,
    ...
    // Card in Settings -> Plugins -> Plugin Settings (if available)
    ...
    exports.apply = function apply(ctx) {
      ...
      // Register both settings.section (left sidebar item) and settings.plugin.item (card)
      if (ctx.slots) {
