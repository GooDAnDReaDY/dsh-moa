window.__ModuleLoader__.load({
  ...
  factory: (require) => {
            ...
            React.createElement(
              ...
              React.createElement(
                ...
                React.createElement('input', {
                  ...
                  style: { accentColor: 'var(--dsw-alias-brand-primary, var(--dsw-alias-label-primary))', cursor: 'pointer' },
                }),
                t('askQuestionsLabel') || 'Спрашивать уточняющие вопросы при неясных задачах (Опросник)'
              ),
              React.createElement(
                'div',
                { className: 'moa-hint', style: { marginLeft: 22 } },
                t('askQuestionsHint') || 'Если включено: при коротких задачах судья сформирует опросник. Если выключено: MoA сразу переходит к генерации решения.'
              )
            )
          )
        ),
    ...
    function useMoASettings(props) {
      ...
      // Reactive binding to settingsScope if available
      ...
      // Sync from settingsScope snapshot when available
      React.useEffect(() => {
