window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const STYLES = `
      ...
      .moa-select optgroup { font-weight:700; color:var(--dsw-alias-label-secondary); background:var(--dsw-alias-bg-layer-2); }
      .moa-select option { font-weight:normal; color:var(--dsw-alias-label-primary); background:var(--dsw-alias-bg-layer-3); padding:4px 8px; }
    ...
    function MoAEditor({
      ...
      const groupedModels = React.useMemo(() => {
        ...
        const addOpt = (provider, model, label) => {
          const prov = provider || 'other'
          const key = `${prov}:${model}`
          if (!map.has(prov)) map.set(prov, new Map())
          const provMap = map.get(prov)
          if (!provMap.has(key)) {
            provMap.set(key, { key, provider: prov, model, label: label || key })
          }
        }

        // Add from discovered available models
        for (const m of availableModels) {
          if (m && m.provider && m.model) {
            addOpt(m.provider, m.model, m.label)
          }
        }

        // Always include current aggregator and references even if offline
        if (currentPreset?.aggregator?.provider && currentPreset?.aggregator?.model) {
          addOpt(currentPreset.aggregator.provider, currentPreset.aggregator.model)
        }
        for (const r of (currentPreset?.reference_models || [])) {
          if (r?.provider && r?.model) {
            addOpt(r.provider, r.model)
          }
        }

        // Convert Map to array of groups
        const groups = []
        for (const [prov, itemsMap] of map.entries()) {
      ...
      const setReferenceModel = (idx, key) => {
        ...
        updateCurrentPreset((p) => {
          ...
          refs[idx] = { provider: prov || '', model: rest.join(':') || '' }
      ...
      const setReferenceCount = (targetCount) => {
        updateCurrentPreset((p) => {
          ...
          if (refs.length > targetCount) {
            ...
            // Find pool of all items
            const allItems = groupedModels.flatMap((g) => g.items)
            while (refs.length < targetCount) {
              const fallback = allItems[refs.length % (allItems.length || 1)] || { provider: 'opencode-go', model: 'deepseek-v4-flash' }
      ...
      const renderModelOptions = () => {
        return groupedModels.map((g) =>
          React.createElement(
            'optgroup',
            { key: g.provider, label: g.title },
            g.items.map((opt) =>
              React.createElement('option', { key: opt.key, value: opt.key }, opt.label || opt.key)
            )
          )
        )
      }
