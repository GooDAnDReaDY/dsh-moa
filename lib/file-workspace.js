...
export async function writeCandidateWorkspace(baseDir, candidateIndex, files = []) {
  ...
  const sub = /^\d+$/.test(targetStr) ? `candidate-${targetStr}` : targetStr
  const candidateDir = path.join(baseDir, '.moa', sub)
...
export async function promoteCandidateWorkspace(baseDir, winningIndex, options = { keepMoa: false }) {
  ...
  const sub = /^\d+$/.test(targetStr) ? `candidate-${targetStr}` : targetStr
  const winningDir = path.join(baseDir, '.moa', sub)
  ...
  try {
    ...
    // Best-effort Time Machine checkpoint before overwriting files in baseDir
 ...
 * Uses Node.js in-memory vm.Script for JS (with ES module export/import transformation) and JSON.parse for JSON.
 */
...
export async function readCandidateFiles(baseDir, candidateTarget) {
  ...
  const sub = /^\d+$/.test(targetStr) ? `candidate-${targetStr}` : targetStr
  const targetDir = path.join(baseDir, '.moa', sub)
...
/**
 * Computes a standard line-by-line diff between two text strings using LCS.
...
export function computeLineDiff(oldText = '', newText = '') {
  ...
  // Safety bound to avoid polynomial blowup on enormous files
  if (n > 2000 || m > 2000) {
    const res = []
    for (let i = 0; i < n; i++) res.push({ type: 'removed', line: oldLines[i], oldNum: i + 1 })
    for (let j = 0; j < m; j++) res.push({ type: 'added', line: newLines[j], newNum: j + 1 })
    return res
  }
  ...
  while (i > 0 || j > 0) {
    if (i > 0 && j > 0 && oldLines[i - 1] === newLines[j - 1]) {
      stack.push({ type: 'same', line: oldLines[i - 1], oldNum: i, newNum: j })
      i--
      j--
    } else if (j > 0 && (i === 0 || dp[i][j - 1] >= dp[i - 1][j])) {
      stack.push({ type: 'added', line: newLines[j - 1], newNum: j })
      j--
    } else if (i > 0 && (j === 0 || dp[i][j - 1] < dp[i - 1][j])) {
      stack.push({ type: 'removed', line: oldLines[i - 1], oldNum: i })
      i--
    }
  }
