...
export const inject = ['settings', 'webServer', 'llm', 'credentials', 'sessions', 'agents', 'commands']
...
export function apply(ctx, config) {
  ...
  // Register native DSH slash command: /moa
  ctx.inject(['commands'], (cctx) => {
    cctx.effect(() => {
      return cctx.commands.register({
        name: 'moa',
        description: 'Run task through Mixture of Agents architecture',
        input: { hint: '[<preset>] <prompt>' },
        recordInput: true,
        handler: async (invocation) => {
          ...
          if (!raw) {
            return {
              kind: 'error',
              text: 'Prompt is required for /moa. Usage: /moa [<preset>] <prompt>',
            }
          }
          ...
          if (cfg.enabled === false) {
            return {
              kind: 'error',
              text: 'Mixture of Agents is currently disabled in plugin settings.',
            }
          }

          const parsed = parseMoACommand('/moa ' + raw, cfg.presets || [])
          ...
          const targetPreset =
            (cfg.presets || []).find((p) => p.name === parsed.presetName) ||
            cfg.presets?.[0]
          ...
          try {
            ...
            const formattedText = `[Mixture of Agents — Preset: ${targetPreset?.name || 'default'} | Judge: ${moaResult.aggregator}]\n\n${moaResult.content}`
              ...
              try {
                invocation.agent.followup({
                  ...
                  content: [
                    {
                      type: 'text',
                      text: `Пользователь запросил задачу через Mixture of Agents (/moa):\n${parsed.prompt}\n\nСинтезированное решение ансамбля моделей (Судья: ${moaResult.aggregator}):\n${moaResult.content}\n\nИспользуй это решение, чтобы продолжить диалог или предоставить пользователю финальный ответ.`
                    }
                  ],
                  source: { kind: 'user' }
                })
              } catch (e) {
                console.warn('[dsh-moa] agent.followup error:', e)
              }
            }
            ...
            return {
              kind: 'error',
              text: 'MoA execution failed: ' + (err?.message || String(err)),
            }
          }
        },
      })
    }, 'dsh-moa: commands.register')
  })
