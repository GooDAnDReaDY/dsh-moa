    ...
    return ctx.webServer.register({
      ...
      handler: (_req, res) => {
        ...
        const add = (prov, mod) => {
          ...
          seen.add(key)
          result.push({ provider: prov, model: mod, label: key })
        }

        try {
          const s = ctx.get ? ctx.get('settings') : ctx.settings
          if (s && typeof s.get === 'function') {
            const defModel = s.get('agent-default-model')
            if (defModel?.provider && defModel?.model) {
              add(defModel.provider, defModel.model)
            }

            const piAi = s.get('llm-pi-ai')
          ...
          console.warn('[dsh-moa] Error reading settings models:', e)
        }

        try {
          if (ctx.llm) {
            if (typeof ctx.llm.listModels === 'function') {
              const list = ctx.llm.listModels() || []
              for (const item of list) {
                if (typeof item === 'string') {
                  const parts = item.split(':')
                  if (parts.length >= 2) add(parts[0], parts.slice(1).join(':'))
                  else add('default', item)
            ...
            if (typeof ctx.llm.listProviders === 'function') {
              ...
              for (const p of provs) {
                const pId = typeof p === 'string' ? p : p?.id || p?.provider
                for (const m of (p?.models || [])) {
                  const mId = typeof m === 'string' ? m : m?.id
                  if (pId && mId) add(pId, mId)
                }
              }
            }
          }
        } catch (e) {
          console.warn('[dsh-moa] Error reading ctx.llm models:', e)
        }
