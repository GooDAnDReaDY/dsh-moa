...
export function apply(ctx, config) {
        ...
        handler: async (invocation) => {
          ...
          if (invocation?.agent && typeof invocation.agent.followup === 'function') {
            invocation.agent.followup({
              role: 'user',
              content: [{ type: 'text', text: `/moa ${raw}` }],
              source: { kind: 'user' },
            })

            return {
              kind: 'success',
              text: '🧠 Mixture of Agents запущен в текущей сессии...',
            }
          }
          ...
          try {
            const moaResult = await runMoAPipeline({
              userPrompt: parsed?.prompt || raw,
              messages: [],
              preset: targetPreset,
              callLlm,
            })

            const formattedText = formatMoAResponse({
              moaResult,
  ...
  const moaPendingAgents = new WeakSet()
  ...
  ctx.on('agent/pre-step', async (event, next) => {
    ...
    if (userText.startsWith('/moa')) {
      const cfg = getConfig()
      if (cfg.enabled !== false) {
        if (event.agent) moaPendingAgents.add(event.agent)
      }
    }
  ...
  ctx.on('agent/request', async (payload, next) => {
    ...
    if (payload.agent && moaPendingAgents.has(payload.agent)) {
      moaPendingAgents.delete(payload.agent)
      return {
        ...resolved,
        provider: 'moa-runner',
        model: 'ensemble',
