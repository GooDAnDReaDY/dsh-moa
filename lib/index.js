...
export function apply(ctx, config) {
    ...
    if (sctx.settings && typeof sctx.settings.configure === 'function') {
      try {
        ...
        if (typeof sctx.effect === 'function') {
          sctx.effect(() => dispose, 'dsh-moa: settings policy')
        }
      } catch {
  ...
  const saveConfig = async (newFields) => {
    localConfig = { ...localConfig, ...newFields }
    if (ctx.settings && typeof ctx.settings.replace === 'function') {
      try {
        await ctx.settings.replace(NS, newFields)
      } catch (err) {
        console.warn('[dsh-moa] Settings persistence warning:', err?.message || err)
      }
    } else if (ctx.settings && typeof ctx.settings.update === 'function') {
      try {
        await ctx.settings.update(NS, newFields)
      } catch (err) {
        console.warn('[dsh-moa] Settings update warning:', err?.message || err)
      }
    }
    return localConfig
  }
