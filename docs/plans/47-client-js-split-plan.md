...
## Актуализация решения (2026-09-17, issue #71)

В рамках ужесточения кодовой базы и решения issue #71:
1. **Серверная декомпозиция выполнена в полном объёме**:
   - `lib/index.js` (было 722 строки) декомпозирован: HTTP-маршруты вынесены в `lib/routes.js` (408 строк), `lib/index.js` сокращён до 334 строк.
   - `lib/moa-runner.js` (было 794 строки) декомпозирован: параллельный запуск кандидатов и transient-retry вынесены в `lib/moa-candidates.js` (225 строк), `lib/moa-runner.js` сокращён до 578 строк.
   - Все бэкенд-модули укладываются в лимит < 600 строк.

2. **Клиентская половина `lib/client.js` (1812 строк)**:
   - Проведён сравнительный анализ со сборочной схемой `dsh-cron` / `dsh-tts` (конкатенация `lib/client-src/*.js` скриптом `build-client.mjs`).
   - Конкатенация фрагментов не решает требование DSH combo-loader к отдаче единственного собранного файла `lib/client.js` (в npm pack и рантайм всё равно попадает единый бандл на ~1800 строк).
   - Введение искусственного сборочного скрипта создаёт риск рассинхронизации исходников и сгенерированного файла, усложняет отладку и не даёт архитектурного выигрыша.
   - Принято решение: сохранить `lib/client.js` единым целостным модулем в соответствии с пунктом 5 issue #71 («Если что-то решено оставить цельным — зафиксировать причину в этой issue»).
